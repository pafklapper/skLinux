...
# dit is helaas slechts een placeholder :)
...
echo -e "## Uncomment to disable submenu\nGRUB_DISABLE_SUBMENU=y" >> /mnt/etc/default/grub && \
...
arch-chroot /mnt systemctl enable netctl-auto@wlan0 && arch-chroot /mnt systemctl enable systemd-resolved && cp $installationDirectory/networks/* /etc/netctl/
check_fail $?
